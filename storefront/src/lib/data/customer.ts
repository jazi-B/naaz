...
export async function signup(_currentState: unknown, formData: FormData) {
  ...
  try {
    const regRes = await sdk.auth.register('customer', 'emailpass', {
      email: customerForm.email,
      password: password,
    })
    const token = typeof regRes === 'string' ? regRes : (regRes as any)?.token
    ...
    const loginToken = typeof loginRes === 'string' ? loginRes : (loginRes as any)?.token
