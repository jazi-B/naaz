...
const SALES_CH_ID = 'sc_01M1CX2VY7SEJKKQ6SC2ZPCZRE';
...
async function main() {
  console.log('--- CLEANING UP DUMMY SEED PRODUCTS FROM DATABASE ---');
  const pgClient = new Client('postgres://postgres:postgres@localhost:5432/medusa_naaz');
  await pgClient.connect();
  try {
    const delRes = await pgClient.query("DELETE FROM product WHERE title LIKE 'Medusa%'");
    console.log(`Deleted ${delRes.rowCount || 0} dummy demo products from local database.`);
  } catch (e) {
  ...
  console.log(`\nParsed ${groups.length} unique NAAZ Handbag products from Markaz CSV`);
  ...
  console.log(`Importing ${productsPayload.length} products to Local Medusa at http://localhost:9000...`);
  ...
  console.log('\n✅ All NAAZ Handbag products successfully imported into Local Medusa!');
  console.log('Refresh Admin Dashboard: http://localhost:9000/app/products');
}
